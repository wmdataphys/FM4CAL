...
def main(config,args):
    ...
    dataset = ECAL_Chunked_Dataset(test_files[-2:],max_seq_length=msl,
    ...
    for i, data in enumerate(dataloader):
        pos, ene, initial_energy, material_index, initial_energy_t = data
        pos = pos.to(device).long()
        ene = ene.to(device).long() if digitize_energy else ene.to(device).float()
        ...
        with torch.inference_mode():
            ...
            true_energies = ene.detach().cpu().numpy()


            #print(generated_indices.shape, generated_energies.shape)
            ...
            for b in range(generated_indices.shape[0]):
                ...
                if digitize_energy:
                    ...
                    buffers["ene_t"].append(true_energies[b].astype(energy_dtype, copy=False))
                else:
                ...
                #buffers["material_index"].append(dataset.material_list[material_index[b].item()])
